      ...
      <v-dialog v-model="dialog2" persistent max-width="600px">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container>
              <v-switch
                v-model="multivariate"
                class="mx-2"
                :label="`${multivariate_text.toString()}`"
                @change="cambio_nombre($event)"
              ></v-switch>
              <v-row>
              ...
              <v-row>
                ...
                <v-col cols="12">
                  <v-text-field label="Main TS" v-model="ts_data_data" required></v-text-field>
                </v-col>
                
              </v-row>
               
              
              <v-row v-if="multivariate">
              ...
              <v-btn v-if="multivariate" @click="add" class="primary">add</v-btn>  
...
export default {
  ...
  methods: {
        ...
        cambio_nombre (event) {
          console.log(event)
          if (this.multivariate == true) {
            this.multivariate_text = 'multivariate'
          }
          else {
            this.multivariate_text = 'univariate'
          }
        },
    ...
    timecop_realtime: function (event) {
      ...
      if (this.multivariate_text == 'univariate') {
        ...
        this.ts_data_sent.timeseries.push(temporal2_ts)
      }
      ...
      axios.post(this.timecop_url+'/back_'+ this.multivariate_text, this.ts_data_sent, {headers: {'content-type': 'application/json'}})
                .then((response) => {
